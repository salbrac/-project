...
class Attack:
    ...
    def attack(self, audio, lengths, target, finetune=None):
        sess = self.sess
        # target is the index of toks! for example is t is 20 index space is 0! target is list [list]
        ...
        for i in range(MAX):
            ...
            if i % 10 == 0:  # new is wav message
                ...
                for out, logits in lst:
                    ...
                    res2 = np.argmax(logits, axis=2).T  # deepspeech message
                    res2 = ["".join(toks[int(x)] for x in y[:(l - 1) // 320]) for y, l in zip(res2, lengths)]
                    print("\n".join(res2))
            # end if - 10 times one time!
            ...
            # Actually do the optimization ste   // TODO (time spend!!!)
            ...
            # Report progress 1 times print this internal result!
            ...
            for ii in range(self.batch_size):
                ...
                # rescale constant. loss_fn is "CTC"
                if (self.loss_fn == "CTC" and i % 10 == 0 and res[ii] == "".join([toks[x] for x in target[ii]])) \
                    ...
                    print("index:%d time:%s Worked i=%d ctcloss=%f bound=%f" % (
                        i, time.asctime(time.localtime(time.time())), ii, cl[ii], 2000 * rescale[ii][0]))
                    ...
                    wav.write(dirs + "/_" + self.outdir + "_" + str(i) + ".wav", 16000,
                                  np.array(np.clip(np.round(new_input[ii]),
                                                   -2 ** 15, 2 ** 15 - 1), dtype=np.int16))
                if(i%10==0):
                    wav.write(dirs + "/" + self.outdir + "_" + str(i) + ".wav", 16000,
                                  np.array(np.clip(np.round(new_input[ii]),
                                                   -2 ** 15, 2 ** 15 - 1), dtype=np.int16))
        # end iterator for
        return final_deltas
...
def main():
    ...
    with tf.Session() as sess:
        ...
        # Load the inputs that we're given must be 16000 rateSS and int16 bits
        for i in range(len(args.input)):
            # audio is the wav type (bits - 36(type format!))/2 is the true
        ...
        maxlen = max(map(len, audios))  # iterator get max len of audio of audios
        audios = np.array([x + [0] * (maxlen - len(x)) for x in audios])
        finetune = np.array([x + [0] * (maxlen - len(x)) for x in finetune])
        # the target chars we input!
        phrase = args.target
        ...
        attack = Attack(sess, 'CTC', len(phrase), maxlen,
                        ...
                        outdir=args.outdir)
        # an loop running  attack code! start from it
...
main()
# python3 attack.py
# --in sample-000000.wav
# --target "this is a test" --out adv.wav
# --iterations 1000
